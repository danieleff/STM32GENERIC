...
ILI9488_FSMC::ILI9488_FSMC(void) : Adafruit_GFX(ILI9488_TFTWIDTH, ILI9488_TFTHEIGHT) {
  ...
  */
  tftram.Instance = FSMC_NORSRAM_DEVICE;
  tftram.Extended = FSMC_NORSRAM_EXTENDED_DEVICE;
  /* tftram.Init */
  tftram.Init.NSBank = FSMC_NORSRAM_BANK1;
  tftram.Init.DataAddressMux = FSMC_DATA_ADDRESS_MUX_DISABLE;
  tftram.Init.MemoryType = FSMC_MEMORY_TYPE_SRAM;
  tftram.Init.MemoryDataWidth = FSMC_NORSRAM_MEM_BUS_WIDTH_16;
  tftram.Init.BurstAccessMode = FSMC_BURST_ACCESS_MODE_DISABLE;
  tftram.Init.WaitSignalPolarity = FSMC_WAIT_SIGNAL_POLARITY_LOW;
  tftram.Init.WrapMode = FSMC_WRAP_MODE_DISABLE;
  tftram.Init.WaitSignalActive = FSMC_WAIT_TIMING_BEFORE_WS;
  tftram.Init.WriteOperation = FSMC_WRITE_OPERATION_ENABLE;
  tftram.Init.WaitSignal = FSMC_WAIT_SIGNAL_DISABLE;
  tftram.Init.ExtendedMode = FSMC_EXTENDED_MODE_DISABLE;
  tftram.Init.AsynchronousWait = FSMC_ASYNCHRONOUS_WAIT_DISABLE;
  tftram.Init.WriteBurst = FSMC_WRITE_BURST_DISABLE;
  tftram.Init.PageSize = FSMC_PAGE_SIZE_NONE;
  /* Timing */
  ...
  /* ExtTiming */
  HAL_SRAM_Init(&tftram, &Timing, NULL);
}
